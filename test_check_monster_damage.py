...
class TestCheckMonsterDamage(TestCase):
    ...
    def test_check_monster_damage_change_equivalent_to_level_2(self, mock_damage):
    ...
    @patch('game.MONSTER_DAMAGE', return_value={1: {'level': 1, 'damage': 10}, 2: {'level': 2, 'damage': 10 + 5},
                                                3: {'level': 3, 'damage': 10 + (5 * 2)}})
    def test_check_monster_damage_change_equivalent_to_level_3(self, mock_damage):
        ...
        check_monster_damage(player, monster)
        expected = {'name': 'Zelda', 'type': 'Cat', 'category': 'monster', 'damage': 20}
        actual = monster
        self.assertEqual(expected, actual)
